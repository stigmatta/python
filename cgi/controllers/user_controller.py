...
class UserController(RestController):
    ...
    def do_get(self):
        ...
        if not auth_header:
            self.send_error("Missing 'Authorization' header", 401)
            return
        
        auth_scheme = 'Basic '
        if not auth_header.startswith(auth_scheme):
            self.send_error(f"Invalid 'Authorization' header format: {auth_scheme} only", 401)
            return
        
        credentials = auth_header[len(auth_scheme):]
        if len(credentials) < 7:
            self.send_error("Short 'Authorization' credentials value", 401)
            return
        
        match = re.search(r"[^a-zA-Z0-9+/=]", credentials)
        if match:
            self.send_error(f"Format error (invalid symbol) for credentials {credentials}", 401)
            return
        ...
        try:
            ...
            self.send_error(f"Padding error for credentials {credentials}", 401)
            return
        except Exception:
            self.send_error(f"Decoding error for credentials {credentials}", 401)
            return
        
        if not user_pass:
            self.send_error(f"Decode error for credentials {credentials}", 401)
            return

        if not ':' in user_pass:
            self.send_error("Credential format error", 401)
            return
        ...
        if not user:
            self.send_error("Credentials rejected", 401)
            return
        payload = {
